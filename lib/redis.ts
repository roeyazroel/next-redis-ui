...
interface EnvironmentRedisConfig {
  host: string;
  port: number;
  name?: string;
  username?: string;
...
/**
 * Parse environment variables to extract Redis connection configurations
 * Supports indexed naming: REDIS_HOST_1, REDIS_PORT_1, etc.
 */
function parseEnvironmentConnections(): EnvironmentRedisConfig[] {
  const connections: EnvironmentRedisConfig[] = [];
  const processedIndices = new Set<number>();

  // Look for environment variables with indexed naming
  Object.keys(process.env).forEach((key) => {
    const match = key.match(/^REDIS_HOST_(\d+)$/);
    if (match) {
      const index = Number.parseInt(match[1]);
      if (!processedIndices.has(index)) {
        processedIndices.add(index);

        const host = process.env[`REDIS_HOST_${index}`];
        const port = process.env[`REDIS_PORT_${index}`];

        if (host && port) {
          const config: EnvironmentRedisConfig = {
            host,
            port: Number.parseInt(port),
            name: process.env[`REDIS_NAME_${index}`] || `Redis ${index}`,
            username: process.env[`REDIS_USERNAME_${index}`] || undefined,
            password: process.env[`REDIS_PASSWORD_${index}`] || undefined,
            tls: process.env[`REDIS_TLS_${index}`] === "true",
          };

          if (validateEnvironmentConnection(config)) {
            connections.push(config);
          } else {
            console.warn(
              `Invalid environment Redis configuration for index ${index}`
            );
          }
        }
      }
  ...
  console.log(`Found ${connections.length} environment Redis configurations`);
  return connections;
}

/**
 * Validate an environment Redis configuration
 */
function validateEnvironmentConnection(
  config: EnvironmentRedisConfig
): boolean {
  if (!config.host || !config.host.trim()) {
    console.error("Redis host is required");
    return false;
  }

  if (!config.port || config.port < 1 || config.port > 65535) {
    console.error("Redis port must be between 1 and 65535");
    return false;
  }
...
/**
 * Generate a consistent connection ID for environment connections
 */
function generateConnectionId(config: EnvironmentRedisConfig): string {
  return `env_${config.host}_${config.port}`;
}

/**
 * Initialize Redis connections from environment variables
 * Returns array of successfully connected connection IDs
 */
async function initializeEnvironmentConnections(): Promise<string[]> {
  const envConfigs = parseEnvironmentConnections();
  const connectedIds: string[] = [];

  if (envConfigs.length === 0) {
    console.log("No environment Redis connections configured");
    return connectedIds;
  }

  console.log(
    `Initializing ${envConfigs.length} environment Redis connections...`
  );
  ...
  for (const envConfig of envConfigs) {
    try {
      const connectionConfig: RedisConnectionConfig = {
        ...envConfig,
        id: generateConnectionId(envConfig),
        name: envConfig.name || envConfig.host,
      };

      console.log(
        `Connecting to environment Redis: ${connectionConfig.name} (${connectionConfig.host}:${connectionConfig.port})`
      );

      const client = await createRedisClient(connectionConfig);
      await client.ping(); // Test the connection

      connectedIds.push(connectionConfig.id);
      console.log(`✓ Successfully connected to ${connectionConfig.name}`);
    } catch (error) {
      console.error(
        `✗ Failed to connect to ${envConfig.name || envConfig.host}:`,
        error
      );
  ...
  console.log(
    `Environment Redis initialization complete: ${connectedIds.length}/${envConfigs.length} connections established`
  );
  return connectedIds;
}

/**
 * Get environment connection configurations as RedisConnectionConfig objects
 */
function getEnvironmentConnectionConfigs(): RedisConnectionConfig[] {
  const envConfigs = parseEnvironmentConnections();
...
export async function createRedisClient(
  ...
  console.log(
    `Creating new Redis client for ${config.name} (${config.host}:${config.port})`
  );
...
export function getRedisClient(connectionId: string): Redis | undefined {
  ...
  if (client.status !== "ready" && client.status !== "connect") {
    console.warn(
      `Redis client for ${connectionId} is not ready (status: ${client.status})`
    );
  }
...
export async function disconnectRedisClient(
  ...
  if (client) {
    ...
    try {
      ...
      console.error(
        `Error disconnecting Redis client for ${connectionId}:`,
        error
      );
    ...
    switch (type.toLowerCase()) {
      ...
      case "json":
        try {
          ...
          console.warn(
            `RedisJSON module not available, falling back to string parsing for key ${key}`
          );
    ...
    switch (type.toLowerCase()) {
      ...
      case "json":
        try {
          ...
          console.warn(
            `RedisJSON module not available, falling back to string storage for key ${key}`
          );
          // Fallback to string
...
export async function getMemoryStats(client: Redis): Promise<any> {
  try {
    ...
    return {
      ...
      fragmentation: Number.parseFloat(
...
export { getEnvironmentConnectionConfigs, initializeEnvironmentConnections };
